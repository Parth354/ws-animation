...
export class AnimationServer {
  ...
  attach(wss) {
    ...
    this.wss.on('connection', (ws) => {
      if (this.sessions.size >= MAX_CLIENTS) {
        return ws.close(1013, 'Server overloaded');
      }
  ...
  handleMessage(id, raw) {
    ...
    try {
      ...
      return sendError(s.ws, 'Invalid JSON');
    }

    switch (msg.type) {
      case 'START':
        return this.startAnim(id);
      case 'STOP':
        return this.stopAnim(id);
      case 'HEARTBEAT':
        return sendMessage(s.ws, {
          type: 'HEARTBEAT_ACK',
          sessionId: id,
          timestamp: Date.now(),
          payload: { active: s.isActive }
        });
      default:
        return sendError(s.ws, `Unknown type: ${msg.type}`);
    }
  }
